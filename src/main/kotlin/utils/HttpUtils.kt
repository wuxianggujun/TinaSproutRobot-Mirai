...
object HttpUtils {
    ...
    suspend fun verifyCaptcha(cookie: String, code: String): Boolean {
        ...
        if (rsp.isSuccessful) {
            ...
            if (rspCode == null) {
                TinaSproutBotPlugin.logger.error("校验验证码时出现问题$json")
            }
            if (rspCode != 102) {
                TinaSproutBotPlugin.logger.info("验证成功")
                return true
            }
            TinaSproutBotPlugin.logger.info("验证失败")
            return false
        }
        TinaSproutBotPlugin.logger.error("校验验证码时出现问题${rsp.message}")
        return false
    }
