...
object ScriptChallenge {
    fun init() {
        val scriptPath = TinaSproutBotPlugin.dataFolder.resolve("scripts")
...
if  answer == "123":
    ...
    fun getQuestion(groupId: Long): String? {
        val file = TinaSproutBotPlugin.dataFolder.resolve("scripts").resolve("${groupId}.py")
        if (!file.exists()) {
            return null
        }
    ...
    suspend fun auth(groupId: Long, userId: Long, answer: String): Boolean {
        ...
        if (!rsp.isSuccessful) {
            TinaSproutBotPlugin.logger.error("脚本验证失败 ${rsp.message}")
        }
        val body = rsp.body?.string()
        rsp.close()

        val jsonData = body?.let { Json.parseToJsonElement(it) }
        TinaSproutBotPlugin.logger.info("脚本验证返回结果 $jsonData")
