...
import wxgj.tinasproutrobot.mirai.file.GroupFile;
...
public class JavaPluginMain extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        GlobalEventChannel.INSTANCE.filterIsInstance(BotOnlineEvent.class).filter(event -> event.getBot().getId() == qqBotNo).subscribeAlways(BotOnlineEvent.class, event -> {
            ...
            //自动同意加好友请求
            eventChannel.subscribeAlways(NewFriendRequestEvent.class, e -> {
                e.accept();
            });
            eventChannel.subscribeAlways(BotInvitedJoinGroupRequestEvent.class,e ->{
               e.accept();
            });
            eventChannel.registerListenerHost(new GroupFile());
        });


        File dataFile = resolveDataFile("myDataFile.txt");
        getLogger().info("路径："+dataFile);
